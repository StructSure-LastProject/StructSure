...
class ScanViewModel(private val scanDao: ScanDao, private val resultDao: ResultDao, private val structureId: Long) : ViewModel() {
    ...
    fun fetchSensorsAndStartScan(structureId: Long) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (sensors.isNotEmpty()) {
                ...
                println("Fetched and cached ${sensors.size} sensors")
            } else {
     ...
     */
    fun insertSensorsAndStartScan(rfidChip: RfidChip, sensors: List<GetAllSensorsResponse>) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            println("Inserting ${sensorEntities.size} sensors into the cache")
            sensorCache.insertSensors(sensorEntities)
            println("Sensors in cache: ${sensorCache.getAllSensors().size}")

            startSensorInterrogation(rfidChip)
        }
    }
     ...
     */
    private suspend fun startSensorInterrogation(rfidChip: RfidChip) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            for (i in lastProcessedSensorIndex until sensors.size) {
                ...
                val resultSensor = ResultSensors(
                    ...
                    state = sensor.state, // Ensure this field is set
                ...
                // Check if continueScanning is false after insertion
                if (!continueScanning) {
                    lastProcessedSensorIndex = i // Saving the last processed sensor index
                    return@launch
                }
                ...
                scanDao.updateSensorState(sensor.controlChip, sensor.measureChip, sensor.state)
            }
        }
    }
    /**
